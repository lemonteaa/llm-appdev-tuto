...
console.log(f"Container started. Local folder: {os.path.join( os.getcwd(), AGENT_LOCAL_HOME, formatted_date_time)}")

"""
>>> res = cli.images.build(fileobj= io.BytesIO( str.encode(build_str)) )
>>> res
(<Image: ''>, <itertools._tee object at 0x7260ba72ab00>)
>>> res[0]
<Image: ''>
>>> res[0].id
'sha256:630679df3cb1fcc5845b5bc2ea8136b42e05295bb70d72da9c1174144a30bf89'
>>> res[0].labels
...
'sha256:630679df3cb1'
>>> res[1]
<itertools._tee object at 0x7260ba72ab00>
>>> for h in res[1]:
...     print(h)
... 
{'stream': 'Step 1/17 : FROM nikolaik/python-nodejs:latest'}
{'stream': '\n'}
{'stream': ' ---> f4d962f76d8c\n'}
{'stream': 'Step 2/17 : ARG git_user_name=AI-Agent'}
{'stream': '\n'}
{'stream': ' ---> Using cache\n'}
{'stream': ' ---> 02a96ccdcd70\n'}
...
{'aux': {'ID': 'sha256:630679df3cb1fcc5845b5bc2ea8136b42e05295bb70d72da9c1174144a30bf89'}}
{'stream': 'Successfully built 630679df3cb1\n'}
>>> 
"""
...
tool_descs = {
    ...
    "list_command_shell_sessions": "",
    "poll_interactive_command_shell_output": "",
...
def read_single_file_enriched(filepath, uniform_format=True):
    ...
    # Frontend hook
    rich_print_source_code(console=console, content="\n".join(original_content))
...
# Example usage:
#filepath = 'App.jsx'
#content = read_file_with_line_numbers(filepath, uniform_format=True)
#print(content)

def write_files_unified_diff_after_editmode(repo_root, file_content):
    # Frontend hook
    rich_print_source_code(console=console, content=file_content, lang="udiff")
    # Main
    obj_stream = io.BytesIO()
    tmp_file_name = None
    with tarfile.open(fileobj=obj_stream, mode='w|') as tmp_tar, tempfile.NamedTemporaryFile(mode="w+", encoding="utf-8") as tmp_file:
        tmp_file.write(file_content)
        tmp_file.flush()
        obj_info = tmp_tar.gettarinfo(fileobj=tmp_file)
        tmp_file_name = obj_info.name
        tmp_tar.addfile(obj_info, tmp_file)
    sandbox.container.put_archive('/tmp/', obj_stream.getvalue())
...
def write_single_file_vanilla_fallback_after_editmode(filepath, file_content):
    ...
    with open(os.path.join(USER_HOME_DIR, filepath), mode="w+", encoding="utf-8") as f:
...
From a technical perspective, there are three types of tools:
1. Ordinary tool - call them as usual
2. Edit mode tool - when one or more of the argument as originally designed involved complex, multiline string, or when nested/multi-language string escape problem is a significant concern.
3. Indicator tool - semi-dummy tool used as either agent lifecycle management, or for end user interaction.

The idea of an edit mode tool is to remove those problematic arguments, and instead hand control back to you in the next round, where you will write the long argument out.

Example: As originally designed, a write file tool may be like write_file_naive(filename, file_content). In edit mode, the tool will have no argument, and a typical interaction may look like:

(AI round) Tool call: write_file()
Tool call reply: Entering edit mode. Please emit the file name in a markdown style preamble, followed by file content. Make no other output and no prose commentary.
(AI round)
--
filename: main.py
--
...
while True:
    print("Hello")
    time.sleep(1)
Tool call reply: Recieved successfully.
