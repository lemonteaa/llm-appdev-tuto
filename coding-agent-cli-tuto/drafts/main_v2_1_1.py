...
Always reason in natural language prose first (with markdown formating if necessary). For the actual tool call in each agent turn, system will let you do so in the next chat turn.
That is, the message/chat structure will be enforced as follow (each list item is a separate message):

- System prompt
- User request
- Assistant reasoning, agent turn 1
- Assistant tool call, agent turn 1
- Tool call result 1
- ...
- Tool call result n
- System generated user message (indicate to you that you can perform next agentic turn)
- Assistant reasoning, agent turn 2
- ......

etc.

It is important that you do NOT generate tool calls syntax during the "Assistant reasoning" chat turn. If you want to display source code during reasoning chat turn, you may do so using standard markdown codeblock instead.
...
def main_agent_loop():
    ...
    while not done:
        # Call LLM
        res = main_call_llm(conversation, tool_required=False)
        ...
        if res.choices[0].finish_reason == 'tool_calls':
            ...
            for idx, tool_call in enumerate(res.choices[0].message.tool_calls):
                ...
                if "editmode" in tool_meta:
                    ...
                    f_args[tool_meta["param_name"]] = additional_arg
